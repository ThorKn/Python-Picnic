...
class LowMC(object):
    ...
    def private_key(self, priv_key: Optional[bytes] = None) -> None:
        ...
        """
        if (priv_key is None):
            tmp_key = os.urandom(self.__keysize_bytes)
    ...
    def mpc_matrix_mul_keys(self, inputs, r, players):
        """ToDo."""
        inputs_int = [0, 0, 0]
        result = [0, 0, 0]
        for player in range(players):
            inputs_int[player] = int(inputs[player])
            for mat_row in self.__round_key_mats[r]:
                row = mat_row & inputs_int[player]
                bit = bin(row).count("1") % 2
                result[player] = (result[player] << 1) | bit
        return result

    def mpc_matrix_mul_lin(self, inputs, r, players):
        """ToDo."""
        inputs_int = [0, 0, 0]
        result = [0, 0, 0]
        for player in range(players):
